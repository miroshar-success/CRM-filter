...
$(document).ready(function() {
    $('#custom_fields\\[proposal\\]\\[31\\]').val(1);
    $('.table.estimate-items-table > * > tr > :where(*:nth-child(8)) input[name="quantity"] ').focusout(function() {
        ...
        $('#custom_fields\\[proposal\\]\\[31\\]').val(newValue); 
...
$(document).ready(function() {
      setTimeout(function() {
    // var firstSpanText = $('h4.tw-text-lg.tw-text-neutral-700 span:first').text().trim().replace(/\s/g, '');
    var firstSpanText = $('h4.tw-text-lg.tw-text-neutral-700 span:first').text().trim().match(/\d+/)[0];
    $('#custom_fields\\[proposal\\]\\[48\\]').val(firstSpanText);
...
//                    $('multiselect[name="custom_fields[proposal][30][]').val(response.network);
                    // $('.custom-field-multi-select"]').val(response.network);

// Assuming response.network is a string containing comma-separated values
var selectedOptions = response.network.split(',');

// Iterate through each option in the select element
$('select[name="custom_fields[proposal][30][]"] option').each(function() {
    // Check if the option's value is in the selectedOptions array, ignoring case
    if (selectedOptions.some(option => option.trim().toLowerCase() === $(this).val().toLowerCase())) {
        // If the option's value is in the selectedOptions array, add the selected attribute
        $(this).attr('selected', true);
        $(this).prop('selected', true);
    } else {
        // If the option's value is not in the selectedOptions array, remove the selected attribute
...
$('select[name="custom_fields[proposal][30][]"]').selectpicker('refresh');
